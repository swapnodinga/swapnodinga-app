...
export default function AdminDashboard() {
  ...
  // Helper for synchronized maturity and interest logic
  const getMaturityData = (amount: number, rate: number, start: string, months: number) => {
    ...
    // Exact day-diff calculation matching FixedDepositPage
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      setStats({
        ...
        totalFixedDeposits: totalActivePrincipal, // Current Working Capital
        totalInterest: totalRealizedInterest,
        totalFund: totalInstallmentSum + totalRealizedInterest // Matches Member Equity Logic
      });
      ...
      const chartData = Array.from(memberAggregates.entries()).map(([mId, data]) => {
        ...
        const interestShare = totalFinishedPrincipal > 0 
          ? (totalRealizedInterest / totalFinishedPrincipal) * installmentAmount 
          : 0;
  ...
  const financialMixData = [
    ...
    { label: 'REALIZED INT.', val: stats.totalInterest, color: '#8b5cf6' },
    { label: 'SOCIETY NET', val: stats.totalFund, color: '#0f172a' }
  ];
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50/50 min-h-screen">
      <div className="flex justify-between items-center bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
        <div>
          <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Society Administration</h1>
          <p className="text-xs font-bold text-slate-400 mt-0.5 uppercase tracking-widest">Financial Oversight Dashboard</p>
        </div>
        <Select value={selectedYear} onValueChange={setSelectedYear}>
          ...
          <SelectContent className="rounded-xl border-slate-200">
            {[2024, 2025, 2026, 2027].map(y => <SelectItem key={y} value={y.toString()} className="font-medium">{y} Fiscal Year</SelectItem>)}
          </SelectContent>
        </Select>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard title="Net Society Value" value={`৳${Math.round(stats.totalFund).toLocaleString()}`} icon={Landmark} className="bg-slate-900 text-white border-none rounded-2xl" />
        <StatCard title="Total Installments" value={`৳${Math.round(stats.totalInstalments).toLocaleString()}`} icon={PiggyBank} className="bg-white border-b-4 border-emerald-500 rounded-2xl" />
        <StatCard title="Active FD Capital" value={`৳${Math.round(stats.totalFixedDeposits).toLocaleString()}`} icon={Wallet} className="bg-white border-b-4 border-blue-500 rounded-2xl" />
        <StatCard title="Realized Interest" value={`৳${Math.round(stats.totalInterest).toLocaleString()}`} icon={TrendingUp} className="bg-white border-b-4 border-purple-500 rounded-2xl" />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="border-none shadow-sm rounded-2xl overflow-hidden bg-white border border-slate-200">
          ...
          <CardContent className="h-[400px] pt-10">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={financialMixData} margin={{ top: 10, bottom: 10 }}>
                ...
                <Bar dataKey="val" radius={[8, 8, 0, 0]} barSize={50}>
                  ...
                  <LabelList dataKey="val" position="top" formatter={(v: any) => `৳${(v/1000).toFixed(0)}k`} style={{ fontSize: '11px', fontWeight: '900', fill: '#1e293b' }} offset={10} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        ...
        <Card className="border-none shadow-sm rounded-2xl overflow-hidden bg-white border border-slate-200">
          <CardHeader className="border-b bg-slate-50/50 py-4">
            <CardTitle className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em]">Member Asset Leaderboard</CardTitle>
          </CardHeader>
          <CardContent className="h-[400px] pt-10">
            {memberChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={memberChartData} margin={{ top: 10, bottom: 10 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  ...
                  <Bar dataKey="total" fill="#10b981" radius={[8, 8, 0, 0]} barSize={40}>
                    <LabelList dataKey="total" position="top" formatter={(v: any) => `৳${(v/1000).toFixed(0)}k`} style={{ fontSize: '10px', fontWeight: '900', fill: '#059669' }} offset={10} />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-slate-400 font-bold uppercase text-xs tracking-widest">
