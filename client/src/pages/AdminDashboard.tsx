    ...
    <div className="p-6 space-y-6 bg-slate-50/50 min-h-screen">
      ...
      <div className="flex justify-between items-center bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
        <div>
          ...
          <p className="text-[10px] font-bold text-emerald-600 mt-0.5 uppercase tracking-[0.2em]">Live Treasury Metrics</p>
        </div>
        <Select value={selectedYear} onValueChange={setSelectedYear}>
          <SelectTrigger className="w-[160px] font-bold shadow-none border-slate-200 rounded-xl h-11">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="rounded-xl border-slate-200">
            {[2024, 2025, 2026, 2027].map(y => <SelectItem key={y} value={y.toString()} className="font-bold">{y} Year</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {/* Net Society Value: Now with White Text for clear visibility */}
        <div className="bg-[#0f172a] p-6 rounded-2xl shadow-xl border-b-4 border-emerald-500 flex flex-col justify-between min-h-[130px]">
           <div className="flex justify-between items-start">
             <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Net Society Value</p>
             <Landmark className="h-5 w-5 text-emerald-400" />
           </div>
           <h2 className="text-3xl font-black text-white font-mono">৳{Math.round(stats.totalFund).toLocaleString()}</h2>
        </div>

        <StatCard title="Total Installments" value={`৳${Math.round(stats.totalInstalments).toLocaleString()}`} icon={PiggyBank} className="bg-white border-b-4 border-emerald-500 rounded-2xl shadow-sm text-slate-900" />
        <StatCard title="Active FD Capital" value={`৳${Math.round(stats.totalFixedDeposits).toLocaleString()}`} icon={Wallet} className="bg-white border-b-4 border-blue-500 rounded-2xl shadow-sm text-slate-900" />
        <StatCard title="Realized Interest" value={`৳${Math.round(stats.totalInterest).toLocaleString()}`} icon={TrendingUp} className="bg-white border-b-4 border-purple-500 rounded-2xl shadow-sm text-slate-900" />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="border-none shadow-sm rounded-2xl overflow-hidden bg-white border border-slate-200">
          ...
          <CardContent className="h-[400px] pt-12"> {/* Added more top padding for labels */}
            <ResponsiveContainer width="100%" height="100%">
              ...
              ]} margin={{ top: 30, bottom: 10 }}> {/* Increased top margin for labels */}
        ...
        <Card className="border-none shadow-sm rounded-2xl overflow-hidden bg-white border border-slate-200">
          ...
          <CardContent className="h-[400px] pt-12">
            {memberChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={memberChartData} margin={{ top: 30, bottom: 10 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
