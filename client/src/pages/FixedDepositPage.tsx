...
export default function FixedDepositPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Updated payload to match required DB columns and logic
    const payload = {
      amount: Number(formData.amount),
      start_date: formData.start_date,
      interest_rate: Number(formData.interest_rate),
      tenure_months: Number(formData.tenure_months),
      // Computed helper fields for the database
      month: dateObj.toLocaleString('default', { month: 'long' }), 
      year: dateObj.getFullYear().toString(), 
      status: "Active",
      // Link to the current authenticated user/member
    ...
    try {
      ...
      // Reset only the amount to make repeat entries faster
      setFormData({ ...formData, amount: "" })
    } catch (err: any) {
      console.error("Save failed:", err)
      alert("Error saving deposit. Ensure your session is active.")
    } finally {
